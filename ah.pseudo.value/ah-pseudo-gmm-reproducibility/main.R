...
# Resolve the repository root without relying on the user's working directory.
...
# Users may override this path with the AH_GMM_OUTPUT_DIR environment variable.
output_dir <- Sys.getenv(
  "AH_GMM_OUTPUT_DIR",
  unset = file.path(project_dir, "results", "latest")
)
...
simulation_seed <- as.integer(Sys.getenv("AH_GMM_SEED", unset = "2026"))
...
# Override with AH_GMM_NSIM for a quick test, for example:
#   AH_GMM_NSIM=10 Rscript main.R
nsim <- as.integer(Sys.getenv("AH_GMM_NSIM", unset = "1000"))
...
run_PO_regular <- TRUE
run_PO_group_specific <- TRUE
run_PO_covariate_dependent <- FALSE
run_AHCox <- TRUE
run_GMM <- TRUE
run_GMM_PO_GS <- TRUE
run_PO_group_specific_quantileCensoring <- TRUE
run_GMM_PO_GS_quantileCensoring <- TRUE
...
# Stable filename consumed by make_table2.R and make_table3.R.
write.csv(
  final_summary_rounded,
  file.path(output_dir, "simulation_summary_long.csv"),
  row.names = FALSE
)
...
cat("  simulation_summary_long.csv\n")
