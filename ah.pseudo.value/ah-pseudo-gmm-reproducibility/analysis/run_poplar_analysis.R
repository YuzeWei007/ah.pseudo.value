...
# PO-only / AH-only / Stacked-GMM-AH / group-specific PO extensions
#
# Purpose:
...
#   3. Stacked-GMM-PO-AH
#   4. PO-GS, treatment-group-specific PO
#   5. PO-GS-QC, censoring-risk-quartile group-specific PO
#   6. Stacked-GMM-POGS-AH
#   7. Stacked-GMM-POGS-QC-AH
#
# Important:
...
#
# Required files:
#   1. utility.R in the repository root
#   2. an authorized local copy of the POPLAR workbook
#
# Main outputs:
#   1. KM curve PDF
#   2. risk-set table
...
utility_file <- file.path(project_dir, "utility.R")

if (!file.exists(utility_file)) {
  stop("Cannot find utility file: ", utility_file)
}

source(utility_file)

excel_file <- Sys.getenv(
  "POPLAR_DATA_FILE",
  unset = file.path(project_dir, "data", "41591_2018_134_MOESM3_ESM.xlsx")
)

if (!file.exists(excel_file)) {
  stop(
    "Cannot find the POPLAR workbook: ", excel_file,
    "\nProvide an authorized local copy and set POPLAR_DATA_FILE."
  )
}

output_dir <- Sys.getenv(
  "POPLAR_OUTPUT_DIR",
  unset = file.path(project_dir, "results", "poplar")
)
...
make_treatment_group_real <- function(dat) {
  if (!("trt" %in% names(dat))) {
    stop("\nCannot construct treatment-group-specific PO because trt is missing.")
  }
  
  group <- as.integer(dat$trt)
  
  if (length(unique(group)) < 2) {
    stop("\nTreatment group has fewer than two levels.")
  }
  
  group
}

make_censoring_lp_quantile_group_real <- function(dat,
                                                  selected_covariates,
                                                  tau,
                                                  n_groups = 4) {
  covariates4cens <- as.data.frame(dat[, selected_covariates, drop = FALSE])
  ...
  formula_cens <- as.formula(
    paste0(
      "Surv(time_tau, censor_status) ~ ",
      paste(cens_vars, collapse = " + ")
    )
  )
  
  fit <- try(
    coxph(formula_cens, data = df),
    silent = TRUE
  )
  ...
  breaks <- unique(as.numeric(quantile(lp, probs = probs, na.rm = TRUE)))
  ...
  if (length(breaks) == n_groups + 1) {
    ...
    rank_breaks <- unique(as.numeric(quantile(lp_rank, probs = probs)))
    group <- cut(
      lp_rank,
      breaks = rank_breaks,
      include.lowest = TRUE,
...
fit_all_methods_real <- function(dat, selected_covariates, tau) {
  ...
  po_gs <- get_PO_group_specific(
    X = dat$time,
    Delta = dat$status,
    tau = tau,
    group = make_treatment_group_real(dat)
  )
  
  qc_group <- make_censoring_lp_quantile_group_real(
    dat = dat,
  ...
  if (is.null(qc_group)) {
    ...
    po_gs_qc <- get_PO_group_specific(
      X = dat$time,
      Delta = dat$status,
      tau = tau,
      group = qc_group
  ...
  po_gs_fit <- fit_PO_with_se(
    Z = Z,
    po = po_gs,
    start = start
  )
  
  names(po_gs_fit$beta) <- colnames(Z)
  names(po_gs_fit$se) <- colnames(Z)
  
  po_gs_qc_fit <- fit_PO_with_se(
    Z = Z,
    po = po_gs_qc,
    start = start
  )
  
  names(po_gs_qc_fit$beta) <- colnames(Z)
  names(po_gs_qc_fit$se) <- colnames(Z)
  ...
  gmm_pogs_fit <- fit_GMM_real_with_se(
    Z = Z,
    po = po_gs,
    ipcw = ipcw,
    start = start,
  ...
  names(gmm_pogs_fit$beta) <- colnames(Z)
  names(gmm_pogs_fit$se) <- colnames(Z)
  
  gmm_pogs_qc_fit <- fit_GMM_real_with_se(
    Z = Z,
    po = po_gs_qc,
    ipcw = ipcw,
    start = start,
  ...
  names(gmm_pogs_qc_fit$beta) <- colnames(Z)
  names(gmm_pogs_qc_fit$se) <- colnames(Z)
  ...
  list(
    ...
    fits = list(
      "PO-only" = po_fit,
      "PO-GS" = po_gs_fit,
      "PO-GS-QC" = po_gs_qc_fit,
      "AH-only" = list(beta = ah$beta, se = ah$se, fit = ah$fit),
      "Stacked-GMM-PO-AH" = gmm_fit,
      "Stacked-GMM-POGS-AH" = gmm_pogs_fit,
      "Stacked-GMM-POGS-QC-AH" = gmm_pogs_qc_fit
    )
  )
}
...
make_method_regression_table <- function(fit_obj, display_labels, conf_level = 0.95) {
  ...
  for (method_name in names(fit_obj$fits)) {
    ...
    lower <- beta - z * se
    upper <- beta + z * se
    lower_ratio <- exp(as.numeric(lower))
    upper_ratio <- exp(as.numeric(upper))
    ...
    tmp <- data.frame(
      ...
      Lower_95 = lower_ratio,
      Upper_95 = upper_ratio,
      CI_95 = paste0(
        "(",
        sprintf("%.3f", lower_ratio),
        "-",
        sprintf("%.3f", upper_ratio),
        ")"
      ),
      CI_length = upper_ratio - lower_ratio,
...
cat("\n============================================================\n")
cat("Fitting PO-only / PO-GS / PO-GS-QC / AH-only / stacked GMM methods\n")
cat("============================================================\n")
